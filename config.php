...
function getUserOrderDetailKeys($id)
{
    ...
    if ($order->num_rows == 0) {
        ...
        $leftgb = ceil(round(($total - $up - $down) / 1073741824, 2) * 1.176) . " GB";
        $configLinks = "";
...
function getOrderDetailKeys($from_id, $id)
{
    ...
    if ($order->num_rows == 0) {
        ...
        if ($response) {
            ...
            // $leftgb = ( ceil(($total * 1.176) / 1073741824) / ($total / 1073741824) );
            $leftgb = (round(((($total - $up - $down) / 1073741824) * ( ceil(($total * 1.176) / 1073741824) / ($total / 1073741824) )) , 2)) . " GB";
        } else $leftgb = "⚠️";
        $configLinks = "";
        foreach ($acc_link as $acc_link) {
            $configLinks .= ($botState['configLinkState'] != "off" ? "\n <code>$acc_link</code>" : "");
...
function deleteClient($server_id, $inbound_id, $uuid, $delete = 0)
{
    ...
    if ($delete == 1) {
        $dataArr = array(
            'up' => $row->up, 'down' => $row->down, 'total' => $row->total, 'remark' => $row->remark, 'enable' => 'true',
...
function editInboundRemark($server_id, $uuid, $newRemark)
{
    ...
    $dataArr = array(
        'up' => $up, 'down' => $down, 'total' => $total, 'remark' => $newRemark, 'enable' => 'true',
...
function editInboundTraffic($server_id, $uuid, $volume, $days, $editType = null)
{
    ...
    if ($volume != 0) {
        $leftGB = ceil(($total - $up - $down) * 1.176);
    ...
    $dataArr = array(
        'up' => $up, 'down' => $down, 'total' => is_null($total) ? $row->total : $total, 'remark' => $row->remark, 'enable' => 'true',
...
function changeInboundState($server_id, $uuid)
{
    ...
    $dataArr = array(
        'up' => $row->up, 'down' => $row->down, 'total' => $row->total, 'remark' => $row->remark, 'enable' => $newEnable,
        'expiryTime' => $row->expiryTime, 'listen' => '', 'port' => $row->port, 'protocol' => $row->protocol, 'settings' => $settings,
        'streamSettings' => $row->streamSettings, 'sniffing' => $row->sniffing
    );
...
function updateConfig($server_id, $inboundId, $protocol, $netType = 'tcp', $security = 'none', $rahgozar = false)
{
  ...
  "grpcSettings": {
    "serviceName": "' . parse_url($panel_url, PHP_URL_HOST) . '"
        ...
        $dataArr = array(
            'up' => $row->up, 'down' => $row->down, 'total' => $row->total, 'remark' => $remark, 'enable' => 'true',
...
function addUser($server_id, $client_id, $protocol, $port, $expiryTime, $remark, $volume, $netType, $security = 'none', $rahgozar = false, $planId = null)
{
  ...
  "grpcSettings": {
    "serviceName": "' . parse_url($panel_url, PHP_URL_HOST) . '"
        ...
        // trojan
        $dataArr = array(
            'up' => '0', 'down' => '0', 'total' => $volume, 'remark' => $remark, 'enable' => 'true', 'expiryTime' => $expiryTime, 'listen' => '', 'port' => $port, 'protocol' => $protocol, 'settings' => $settings, 'streamSettings' => $streamSettings,
            'sniffing' => '{
      "enabled": true,
      "destOverride": [
        "http",
  ...
  "grpcSettings": {
    "serviceName": "' . parse_url($panel_url, PHP_URL_HOST) . '"
        ...
        // vmess - vless
        $dataArr = array(
            'up' => '0', 'down' => '0', 'total' => $volume, 'remark' => $remark, 'enable' => 'true', 'expiryTime' => $expiryTime, 'listen' => '', 'port' => $port, 'protocol' => $protocol, 'settings' => $settings, 'streamSettings' => $streamSettings, 'sniffing' => $sniffing
        );
    }
