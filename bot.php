...
if (preg_match('/^createAccAmount(\d+)_(\d+)_(\d+)/', $userInfo['step'], $match) && $text != $buttonValues['cancel'] && ($from_id == $admin || $userInfo['isAdmin'] == true)) {
    ...
    for ($i = 1; $i <= $text; $i++) {
        ...
        if (!$response->success) {
            sendMessage('❌ | خطا، نام کانفیگ وارد شده تکراری میباشد ...');
...
if (preg_match('/selectServer(\d+)/', $data, $match) && ($botState['sellState'] == "on" || ($from_id == $admin || $userInfo['isAdmin'] == true))) {
    ...
    if ($respd->num_rows == 0) {
        ...
        $keyboard[] = ['text' => $buttonValues['back_to_main'], 'callback_data' => ($userInfo['temp'] == "agentBuy" ? "agentOneBuy" : ($userInfo['temp'] == "agentMuchBuy" ? "agentMuchBuy" : "buySubscription"))];
        $keyboard = array_chunk($keyboard, 1);
        ...
        if ($response->success) {
            ...
            if (strpos($list, $text)) {
                ...
                $keys = json_encode(['inline_keyboard' => [
                    ...
                    [
                        ['text' => ceil($upload * 1.3333) ?? " ", 'callback_data' => "wizwizch"],
                        ['text' => "√ آپلود √", 'callback_data' => "wizwizch"],
                    ],
                    [
                        ['text' => ceil($download * 1.3333) ?? " ", 'callback_data' => "wizwizch"],
                    ...
                    [
                        ['text' => ceil($leftMb * 1.3333) ?? " ", 'callback_data' => "wizwizch"],
